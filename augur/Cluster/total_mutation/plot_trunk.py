...
def plot_trunk_stability(trunk_location, tt_line, mutations):
    ...
    if epitope == "True":
        ...
        plt.annotate(mutations, (trutnk_location[1], 15), color='black', size='xx-small', rotation='vertical', verticalalignment='center')
    else:
...
def get_translation_info(line1, stability_pathway, trunk_identifier, mutation_identifier, current_number_mutations):
...
def main():
    ...
    stability_pathway = [0]
    for line1 in ddG_mutation_file1:
        if len(line1.split("\t")) > 1:
            current_number_mutations = get_translation_info(line1, stability_pathway, trunk_identifier, mutation_identifier, current_number_mutations)
    plt.show()
